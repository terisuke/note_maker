...
Runtime validation treats Evo X2 over the Tailscale SSH tunnel as the primary heavy-inference path. Local llama.cpp remains a fallback-only measurement path, and scenario reports must include base URL, model, elapsed time, score, and draft length to prevent accidental local-runtime validation. The 2026-05-02 validation passed on Evo X2 and found local fallback quality/model-compatibility gaps; fallback hardening is tracked in Issue [#36](https://github.com/terisuke/note_maker/issues/36).
