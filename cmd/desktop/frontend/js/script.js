document.addEventListener('DOMContentLoaded', () => {
  ...
  const el = {
    ...
    settingsDrawerToggle: document.getElementById('settings-drawer-toggle'),
    settingsDrawerClose: document.getElementById('settings-drawer-close'),
    settingsBackdrop: document.getElementById('settings-backdrop'),
    artifactsRail: document.getElementById('artifacts-rail'),
    artifactsRailToggle: document.getElementById('artifacts-rail-toggle'),
    ...
    historyConversationList: document.getElementById('history-conversation-list'),
  ...
  el.settingsDrawerToggle.addEventListener('click', toggleSettingsDrawer);
  el.settingsDrawerClose.addEventListener('click', () => setSettingsDrawer(false));
  el.settingsBackdrop.addEventListener('click', () => setSettingsDrawer(false));
  ...
  el.historyConversationList.addEventListener('click', openHistoryConversation);
  ...
  if (compactLayoutQuery.addEventListener) {
    compactLayoutQuery.addEventListener('change', () => syncResponsiveLayout(false));
  } else {
    compactLayoutQuery.addListener(() => syncResponsiveLayout(false));
  }

  function toggleSettingsDrawer() {
    setSettingsDrawer(el.settingsDrawerToggle.getAttribute('aria-expanded') !== 'true');
  }

  function setSettingsDrawer(open, restoreFocus = true, moveFocus = true) {
    const modal = compactLayoutQuery.matches;
    document.body.classList.toggle('settings-drawer-open', open && modal);
    document.body.classList.toggle('settings-drawer-collapsed', !open && !modal);
    el.settingsBackdrop.classList.toggle('hidden', !(open && modal));
    el.settingsDrawerToggle.setAttribute('aria-expanded', String(open));
    el.settingsDrawer.setAttribute('aria-modal', String(open && modal));
    el.settingsDrawer.setAttribute('aria-hidden', String(!open));
    if (open && moveFocus) {
      requestAnimationFrame(() => focusFirstDrawerControl());
    } else if (restoreFocus) {
      el.settingsDrawerToggle.focus();
    }
  }

  function focusFirstDrawerControl() {
    const focusable = drawerFocusableControls()[0] || el.settingsDrawer;
    focusable.focus();
  }

  function onGlobalKeydown(event) {
    if (!settingsDrawerModalOpen()) {
      return;
    }
    if (event.key === 'Escape') {
    ...
    if (event.key === 'Tab') {
      trapSettingsDrawerFocus(event);
    }
  }

  function settingsDrawerModalOpen() {
    return document.body.classList.contains('settings-drawer-open')
      && el.settingsDrawer.getAttribute('aria-modal') === 'true';
  }

  function drawerFocusableControls() {
    return Array.from(el.settingsDrawer.querySelectorAll(drawerFocusableSelector))
      .filter((node) => node.getClientRects().length > 0 && node.getAttribute('aria-hidden') !== 'true');
  }

  function trapSettingsDrawerFocus(event) {
    const focusable = drawerFocusableControls();
    if (!focusable.length) {
      event.preventDefault();
      el.settingsDrawer.focus();
      return;
    }
    const first = focusable[0];
    const last = focusable[focusable.length - 1];
    const active = document.activeElement;
    if (event.shiftKey && (active === first || !el.settingsDrawer.contains(active))) {
      event.preventDefault();
      last.focus();
      return;
    }
  ...
  function toggleArtifactsRail() {
    setArtifactsRail(!document.body.classList.contains('artifacts-rail-open'));
  }

  function setArtifactsRail(open) {
    const expanded = !compactLayoutQuery.matches || open;
    document.body.classList.toggle('artifacts-rail-open', open && compactLayoutQuery.matches);
    el.artifactsRailToggle.setAttribute('aria-expanded', String(expanded));
    el.artifactsRail.setAttribute('aria-hidden', String(!expanded));
  }

  function syncResponsiveLayout() {
    if (compactLayoutQuery.matches) {
      setSettingsDrawer(false, false, false);
  ...
  function renderLiveBriefAndDraft(data = {}) {
  ...
  function renderTranscript(data = {}) {
    ...
    updateTranscriptStatus(answers, nextQuestion, Boolean(data.completed));
    syncTranscriptStore(answers, nextQuestion, Boolean(data.completed));
    el.questionLog.scrollTop = el.questionLog.scrollHeight;
  }

  function focusTranscriptLog() {
    requestAnimationFrame(() => {
      el.questionLog.focus({ preventScroll: true });
    });
  }

  function updateTranscriptStatus(answers, nextQuestion, completed) {
    if (completed) {
      el.transcriptStatus.textContent = `${answers.length}件の回答からブリーフを作成しました。`;
      return;
    }
    if (nextQuestion) {
      el.transcriptStatus.textContent = `${answers.length}件の回答済み / 次の質問を表示中`;
      return;
    }
    el.transcriptStatus.textContent = answers.length ? `${answers.length}件の回答済み` : '取材を開始すると質問が表示されます。';
  }

  function syncTranscriptStore(answers, nextQuestion, completed) {
    if (!(window.Alpine && Alpine.store('app'))) {
      return;
    }
    Alpine.store('app').transcript.items = [
      ...answers.map((answer) => ({
        type: 'answer',
        questionId: answerValue(answer, 'question_id', 'QuestionID'),
        flowType: answerValue(answer, 'flow_type', 'FlowType') || 'main',
        content: answerValue(answer, 'content', 'Content') || '',
      })),
      ...(!completed && nextQuestion ? [{
        type: 'question',
        questionId: nextQuestion.id || nextQuestion.ID || '',
        flowType: nextQuestion.flow_type || nextQuestion.FlowType || 'main',
        content: nextQuestion.text || nextQuestion.Text || '',
      }] : []),
    ];
    Alpine.store('app').transcript.streamState = completed ? 'completed' : (nextQuestion ? 'waiting_for_answer' : 'idle');
  }
  ...
  function createTranscriptItem(answer) {
    ...
    editButton.setAttribute('aria-label', `${questionTextForAnswer(answer)} の回答を編集`);
  ...
  function renderPendingQuestion(existingItem, text) {
    ...
    el.questionLog.scrollTop = el.questionLog.scrollHeight;
    updateTranscriptStatus(state.answers || [], { text }, false);
    return item;
  }
  ...
  async function loadWorkflowHistory() {
    ...
    try {
      ...
      // D2-4: mirror left-rail history list to Alpine.store('app').
      if (window.Alpine && Alpine.store('app')) {
        Alpine.store('app').history.articles = JSON.parse(JSON.stringify(normalized.articles));
        Alpine.store('app').history.selectedArticleId = null;
      }
    } catch (error) {
      if (requestId !== state.historyRequestId) {
        return;
      ...
      // D2-4: mirror empty history failure state to Alpine.store('app').
      if (window.Alpine && Alpine.store('app')) {
        Alpine.store('app').history.articles = [];
        Alpine.store('app').history.selectedArticleId = null;
      }
    } finally {
      if (requestId === state.historyRequestId) {
  ...
  function renderHistoryPicker() {
    ...
    renderHistoryConversationList(articles, drafts);
  ...
  function renderHistoryConversationList(articles, drafts) {
    el.historyConversationList.innerHTML = '';
    if (state.historyLoading) {
      const loading = document.createElement('div');
      loading.className = 'history-conversation-empty';
      loading.textContent = '履歴を読み込んでいます...';
      el.historyConversationList.appendChild(loading);
      return;
    }

    const items = historyConversationItems(articles, drafts);
    if (!items.length) {
      const empty = document.createElement('div');
      empty.className = 'history-conversation-empty';
      empty.textContent = state.historyError ? '履歴リストを表示できません。' : '再開できる会話はまだありません。';
      el.historyConversationList.appendChild(empty);
      return;
    }
    ...
    items.forEach((item) => {
      ...
      button.type = 'button';
      button.className = 'history-conversation-item';
      button.dataset.historyKind = item.kind;
      button.dataset.historyId = item.id;
      button.setAttribute('aria-label', `${item.title} を開く`);
      ...
      el.historyConversationList.appendChild(button);
    });
  }

  function historyConversationItems(articles, drafts) {
    const seen = new Set();
    const items = [];
    articles.forEach((article) => {
      if (!article.id) return;
      seen.add(`article:${article.id}`);
      items.push({
        kind: 'article',
        id: article.id,
        title: article.title || article.theme || article.id,
        meta: ['記事', article.status, formatDateTime(article.updatedAt || article.createdAt)].filter(Boolean).join(' / '),
        updatedAt: article.updatedAt || article.createdAt || '',
      });
    });
    drafts.forEach((draft) => {
      if (!draft.id || (draft.articleId && seen.has(`article:${draft.articleId}`))) return;
      seen.add(`draft:${draft.id}`);
      items.push({
        kind: 'draft',
        id: draft.id,
        title: draft.title || draft.id,
        meta: ['下書き', draft.status, formatDateTime(draft.updatedAt || draft.createdAt)].filter(Boolean).join(' / '),
        updatedAt: draft.updatedAt || draft.createdAt || '',
      });
    });
    state.historySessions.forEach((session) => {
      if (!session.id || seen.has(`session:${session.id}`)) return;
      seen.add(`session:${session.id}`);
      items.push({
        kind: 'session',
        id: session.id,
        title: session.title || session.theme || session.id,
        meta: ['取材', session.completed ? '完了' : session.phase, formatDateTime(session.updatedAt || session.createdAt)].filter(Boolean).join(' / '),
        updatedAt: session.updatedAt || session.createdAt || '',
      });
    });
    return items.sort((a, b) => String(b.updatedAt).localeCompare(String(a.updatedAt))).slice(0, 12);
  }

  async function openHistoryConversation(event) {
    const trigger = event.target.closest('[data-history-kind][data-history-id]');
    if (!trigger) {
      return;
    }
    const { historyKind, historyId } = trigger.dataset;
    clearHistorySelection();
    if (historyKind === 'article') {
      el.historyArticleSelect.value = historyId;
      await selectHistoryArticle();
    } else if (historyKind === 'draft') {
      el.historyDraftSelect.value = historyId;
      await selectHistoryDraft();
    } else if (historyKind === 'session') {
      el.historySessionSelect.value = historyId;
      await selectHistorySession();
    }
    await openSelectedHistory();
  }
  ...
  async function selectHistoryArticle() {
    ...
    state.selectedHistoryDraft = null;
    el.historyDraftSelect.value = '';
    // D2-4: mirror selected article to Alpine.store('app').
    if (window.Alpine && Alpine.store('app')) {
      Alpine.store('app').history.selectedArticleId = state.selectedHistoryArticle?.id || null;
    }
    renderHistoryPicker();
    renderHistoryArticleDetail();
  ...
  async function selectHistoryDraft() {
    ...
    // D2-4: mirror draft-driven article selection to Alpine.store('app').
    if (window.Alpine && Alpine.store('app')) {
      Alpine.store('app').history.selectedArticleId = state.selectedHistoryArticle?.id || null;
    }
    el.historyStatus.className = 'history-status loading';
  ...
  function clearHistorySelection() {
    ...
    state.selectedHistoryStyle = null;
    state.selectedHistorySession = null;
    // D2-4: mirror cleared history selection to Alpine.store('app').
    if (window.Alpine && Alpine.store('app')) {
      Alpine.store('app').history.selectedArticleId = null;
    }
    renderHistoryPicker();
  ...
  function renderStyleGuideCard(data) {
    ...
    if (!data || !markdown) {
      ...
      syncArtifactStore('styleGuide', null);
    ...
    const normalized = normalizeHistoryStyle(data);
    state.currentStyleArtifact = normalized;
    syncArtifactStore('styleGuide', {
      id: normalized.id,
      profileId: normalized.profileId || normalized.id,
      guideId: normalized.guideId,
      title: normalized.title || '文体ガイド',
    });
  ...
  function renderBriefCard(brief) {
    ...
    if (!brief) {
      syncArtifactStore('brief', null);
      el.briefCard.className = 'artifact-card empty';
      el.briefCard.textContent = '記事ブリーフはまだありません。取材完了後、または保存済みセッション選択後に表示します。';
      resetBriefVersions();
      return;
    }
    syncArtifactStore('brief', {
      sessionId: currentBriefSessionId(brief),
      theme: briefField(brief, 'theme', 'Theme') || '',
      reader: briefField(brief, 'reader', 'Reader') || '',
      live: Boolean(brief._live),
    });
    ...
    const theme = briefField(brief, 'theme', 'Theme') || (brief._live ? '取材中のブリーフ' : '記事ブリーフ');
    ...
    if (brief._live) {
      el.briefCard.appendChild(createInlineStatus('回答を送るたびに、このブリーフを取材中の内容で更新しています。'));
    }
  ...
  function liveBriefFromInterview(data = {}) {
    const answers = data.answers || state.answers || [];
    const brief = {
      _live: true,
      session_id: data.session_id || state.sessionId,
      parent_session_id: data.parent_session_id || state.parentSessionId,
      persona_id: currentPersonaId(),
      output_format_id: currentFormatId(),
      style_profile_id: state.profileId,
      theme: answerContentForQuestion(answers, 'theme') || answerContentForQuestion(answers, 'title_keywords'),
      reader: answerContentForQuestion(answers, 'reader') || answerContentForQuestion(answers, 'reader_problem'),
      opening_episode: answerContentForQuestion(answers, 'opening_episode') || answerContentForQuestion(answers, 'concrete_example'),
      expected_reader_action: answerContentForQuestion(answers, 'expected_reader_action') || answerContentForQuestion(answers, 'primary_cta'),
      must_include: answerContentForQuestion(answers, 'must_include') || answerContentForQuestion(answers, 'key_takeaway'),
      personal_context: answerContentForQuestion(answers, 'personal_context'),
      exclusions: answerContentForQuestion(answers, 'exclusions'),
      target_length_structure: answerContentForQuestion(answers, 'target_length_structure'),
      tone_stance: answerContentForQuestion(answers, 'tone_stance') || answerContentForQuestion(answers, 'brand_voice'),
      custom_answers: answers.filter((answer) => (answerValue(answer, 'flow_type', 'FlowType') || 'main') !== 'deep_dive_follow_up'),
      deep_dives: answers.filter((answer) => (answerValue(answer, 'flow_type', 'FlowType') || 'main') === 'deep_dive_follow_up'),
    };
    if (!answers.length) {
      brief.reader = '未回答';
      brief.must_include = '取材回答を送ると、ここに要点が反映されます。';
    }
    return brief;
  }

  function answerContentForQuestion(answers, questionId) {
    const answer = answers.find((item) => answerValue(item, 'question_id', 'QuestionID') === questionId);
    return answerValue(answer, 'content', 'Content');
  }

  function renderLiveDraftPreview(brief, mode = 'interview') {
    const markdown = liveDraftPreviewMarkdown(brief);
    el.draftResult.classList.remove('hidden');
    el.evaluationSummary.className = 'evaluation draft-preview-state';
    el.evaluationSummary.innerHTML = mode === 'ready'
      ? '<strong>READY</strong><span>記事ブリーフが完成しました。生成前の下書き骨子を表示しています。</span>'
      : '<strong>INTERVIEW</strong><span>取材回答から下書き骨子を更新しています。</span>';
    el.verificationSummary.textContent = '';
    el.previewContent.innerHTML = marked.parse(markdown);
    el.sectionStatus.textContent = '';
    syncArtifactStore('draft', {
  ...
  function liveDraftPreviewMarkdown(brief = {}) {
    const title = briefField(brief, 'theme', 'Theme') || '取材中の下書き';
    const sections = [
      ['想定読者', briefField(brief, 'reader', 'Reader')],
      ['冒頭の具体例', briefField(brief, 'opening_episode', 'OpeningEpisode')],
      ['必ず含めること', briefField(brief, 'must_include', 'MustInclude')],
      ['読後アクション', briefField(brief, 'expected_reader_action', 'ExpectedReaderAction')],
      ['トーンと立場', briefField(brief, 'tone_stance', 'ToneStance')],
    ...
    const lines = [`# ${title}`, ''];
    if (!sections.length) {
      lines.push('取材回答を送ると、ここに下書きの骨子が表示されます。');
      return lines.join('\n');
    }
  ...
  function syncArtifactStore(key, value) {
    if (!(window.Alpine && Alpine.store('app'))) {
      return;
    }
    Alpine.store('app').artifacts[key] = value;
  }
  ...
  function renderDraft(data) {
    ...
    syncArtifactStore('draftEvaluation', {
      passed: Boolean(passed),
      score,
      failedMetrics,
    });
  ...
  function setAnswerStreaming(active) {
    ...
    if (window.Alpine && Alpine.store('app')) {
      Alpine.store('app').transcript.streamState = active ? 'streaming' : Alpine.store('app').transcript.streamState;
    }
  }
